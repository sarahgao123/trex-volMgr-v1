...
export function SlotRow({ slot, isOwner, onEdit, onDelete, onCheckIn }: SlotRowProps) {
  ...
  // Ensure volunteers array exists and has the correct property name
  const volunteers = slot.volunteer_list || [];
        ...
        <div className="space-y-2">
          ...
          {(slot.start_time || slot.end_time) && (
            <div className="flex items-center space-x-4 text-sm text-gray-500">
              <div className="flex items-center">
                ...
                <span>
                  {slot.start_time && formatLocalDateTime(slot.start_time)}
                  {slot.start_time && slot.end_time && ' - '}
                  {slot.end_time && formatLocalDateTime(slot.end_time)}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center space-x-4 text-sm text-gray-500">
            <div className="flex items-center">
              ...
              <span>
                {volunteers.length} / {slot.capacity} volunteers
              </span>
            </div>
          </div>
                  ...
                  <div key={volunteer.user.id} className="flex items-center justify-between">
                    ...
                    {volunteer.check_in_time && (
                      <span className="text-xs text-gray-400">
                        Checked in: {formatLocalDateTime(volunteer.check_in_time)}
                      </span>
                    )}
                  </div>
                ))}
